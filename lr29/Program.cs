...
void Register()
{
    ...
    Console.WriteLine("Введите Номер телефона(Без знака '+'): ");
...
int Menu()
{
    while (true)
    {
        ...
        Console.WriteLine("9. Добавить задачу,");
...
Console.WriteLine("Заполнить таблицы?(Да, Нет)");
string chk1 = Console.ReadLine();
if (chk1 == "Да")
{
    Data();
    Console.WriteLine("Таблицы заполнены.");
}
else
{
    Console.WriteLine("Заполнение отменено.");
}
while (true)
{
    Console.WriteLine("Выберите: 1. Регистрация, 2. Авторизация (Выберите цифру!)");
        ...
        while (true)
        {
            switch (Menu())
            {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    Console.WriteLine("Введите номер задания: (Число!)");
                ...
                case 9:
                    ...
                    string name_tk = Console.ReadLine();
                    Console.WriteLine("Введите описание задачи:");
                    string descript_tsk = Console.ReadLine();
                    DateTime date = DateTime.Now;
                    Task task1 = new Task()
                    {
                        NameTask = name_tk,
                        DescriptionTask = descript_tsk,
                        DatePub = date,
                        CreatorId = selUser.Userid,
                        AcceptorId = selUser.Userid,
                        Statusid = 1
                    };
                    db.Tasks.Add(task1);
                    db.SaveChanges();
                    TaskList = new(db.Tasks.Include<lr29.Task, Status>((lr29.Task x) => x.Status));
